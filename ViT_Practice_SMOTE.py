...
ImgRatio = int(len(images) * 0.9)
(x_train), (y_train) = ( np.asarray(images[:ImgRatio]), np.asarray(targets[:ImgRatio]) )
(x_test), (y_test) = ( np.asarray(images[ImgRatio:]), np.asarray(targets[ImgRatio:]) )

#SMOTE Here
X_train = x_train.reshape((len(x_train), input_shape[0]*input_shape[1]*input_shape[2]))
x_re, y_train = SMOTE(random_state=42).fit_resample(X_train, y_train)
x_train = x_re.reshape((len(x_re), input_shape[0], input_shape[1], input_shape[2]))
...
#Save the Model
filename = 'ViT'
tf.keras.models.save_model(vit_classifier, filename)
